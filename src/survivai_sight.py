...
class SurvivAI(gym.Env):
                ...
                if action[2] > 0:
                    ...
                    if world_state.number_of_observations_since_last_state > 0:
                        ...
                        if 'LineOfSight' in ob.keys():
                            ...
                            if ob[u'LineOfSight'][u'type'] == 'log':
                                self.harvestWood2(ob)
                            #x = ob.get(u'Pitch',0)
                            #z = ob.get(u'Yaw',0)
                            #print(x)
                            #print(z)
                            #height = 0.625  
                            #direction = 1.0
                            #target_yaw = direction * x * math.atan(old_div(1.0,z)) * 180.0/math.pi
                            #distance = math.sqrt(x*x + z*z)
                            #target_pitch = math.atan(old_div(height,distance)) * 180.0/math.pi

                            #self.pointTo(ob, target_pitch, target_yaw, 0.5)
    ...
    def pointTo(self, ob, target_pitch, target_yaw, threshold):
        ...
        self.agent_host.sendCommand("turn " + str(delta_yaw))    
        self.agent_host.sendCommand("pitch " + str(delta_pitch))
    ...
    def init_malmo(self, agent_host):
        #Set up mission
        my_mission = MalmoPython.MissionSpec( getXML(MAX_EPISODE_STEPS=100,SIZE=10,N_TREES=7), True)
    ...
    def harvestWood2(self, ob):
        print("HARVESTING 2")
        while 'LineOfSight' in ob.keys() and ob[u'LineOfSight'][u'type'] == 'log' and ob[u'LineOfSight'][u'inRange'] == False:
            self.agent_host.sendCommand("move 0.7") #move towards block if we're not there yet
            time.sleep(1)
            world_state = self.agent_host.getWorldState()
            if world_state.number_of_observations_since_last_state > 0:
                msg = world_state.observations[-1].text
        ...
        time.sleep(1)
        self.agent_host.sendCommand("move 0.2") #collect reward
        time.sleep(1)
        print("DONE HARVESTING 2")
