...
The left side hm$$\log\pi_{\theta}(a_{t}|s_{t})$$ represents the probabilities of actions for a given state that the neural net suggests, and the right side (the advantage function) is an estimate of “how good” an action is.
